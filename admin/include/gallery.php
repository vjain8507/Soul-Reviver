...
<table>
    ...
    <tbody>
        <?php
            $run_posts = mysqli_query($con,"select * from gallery order by sno desc");
            $i=1;
            while($row_post = mysqli_fetch_array($run_posts))
            {
                $img_id = $row_post['sno'];
                $img_name = $row_post['imgname'];
                echo "<tr><td>".$i++."</td><td><img src='../../image/gallery/$img_name' width='200px'></td><td><a href='./home.php?delete_image&de=$img_id'><i class='material-icons'>delete_forever</i></a></td></tr>";
            }
        ?>
    </tbody>
</table>