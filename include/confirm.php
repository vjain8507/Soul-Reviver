...
<?php
    ...
    if(isset($_POST['sendmail']))
    {
        ...
        $log_key = $log['log_key'];
        $log_key = md5($log_key.date('F d, Y h:i:s A'));
        ...
        echo "<script>alert('Reset Password Mail Send.')</script><script>window.open('./?confirm&reset','_self')</script>";
    }
    ...
    if(isset($_POST['updatepass']))
    {
        ...
        else
        {
            if($log_password!=$log_cpassword)
                echo "<script>alert('Password Do Not Match. Click On The Link Send On Your Mail And Proceed Accordingly.')</script><script>window.open('./?confirm&reset','_self')</script>";
            else
            {
                include("include/mail.php");
                ...
                echo "<script>alert('Password Changed Successfully.')</script><script>window.open('./?signin','_self')</script>";
            }
        }
    }
?>